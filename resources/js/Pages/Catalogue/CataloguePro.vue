...
<template>
    <Head>
        <title>{{ data?.pageCatalogueProCollection?.items[0].banner?.title }}</title>
        <meta name="description" :content="data?.pageCatalogueProCollection?.items[0].banner?.description">
    </Head>
    <NorthBotanicaLayout>
        <template #banner-label>
            {{ data?.pageCatalogueProCollection?.items[0].banner?.label }}
        </template>
        <template #banner-name>
            {{ data?.pageCatalogueProCollection?.items[0].banner?.title }}
        </template>
        <template #banner-description>
            {{ data?.pageCatalogueProCollection?.items[0].banner?.description }}
        </template>
            ...
            <section
                ...
                <div class="h-3/4 w-full pb-24 border-b border-gray-300 sm:px-10 px-0">
                    <div
                        ...
                        <jet-card-post v-for="card in data?.pageCatalogueProCollection?.items[0].cardCollection?.items"
                        ...
                        <section class="h-1/4" title="banner">
                            <div class="h-full flex flex-col items-start text-start justify-start gap-2 overflow-clip">
                                <div class="font-semibold drop-shadow-xl text-md lg:text-lg w-full lg:w-1/3">
                                    {{ data?.pageCatalogueProCollection?.items[0].section?.label }}
                                </div>
                                <div
                                    class="text-transparent bg-clip-text bg-gradient-to-l from-darkGreen via-darkGreen to-clearGreen text-black drop-shadow-lg py-1 font-mono subpixel-antialiased font-black text-3xl lg:text-4xl">
                                    {{ data?.pageCatalogueProCollection?.items[0].section?.title }}
                                </div>
                            </div>
                        </section>
                        <jet-grid>
                            <div
                                v-for="images in data?.pageCatalogueProCollection?.items[0].section?.imagesCollection?.items"
                                :style="{ backgroundImage: 'url(' + images.url +')' }"
                                class="h-full w-full hover:transition hover:ease-in-out hover:scale-[102%] relative grid gap-4 max-w-full rounded-lg aspect-square bg-cover bg-center"
                            >
